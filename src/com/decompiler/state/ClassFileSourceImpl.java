...
import static com.decompiler.bytecode.analysis.types.ClassNameUtils.getPackageAndClassNames;
...
public class ClassFileSourceImpl implements ClassFileSource2 {
    ...
    public JarContent addJarContent(String jarPath, AnalysisType analysisType) {
        ...
        if (!file.exists()) {
            throw new ConfusedDecompilerException("No such jar file " + jarPath);
        }
        jarPath = file.getAbsolutePath();
        JarContent jarContent = processClassPathFile(file, false, analysisType);
        if (jarContent == null){
            throw new ConfusedDecompilerException("Failed to load jar " + jarPath);
        }
