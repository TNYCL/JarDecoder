...
import com.decompiler.bytecode.analysis.parse.lvalue.ArrayVariable;
...
import com.decompiler.bytecode.analysis.stack.StackEntryHolder;
...
import com.decompiler.bytecode.analysis.variables.Ident;
import com.decompiler.bytecode.analysis.variables.Slot;
import com.decompiler.bytecode.analysis.variables.VariableFactory;
import com.decompiler.bytecode.analysis.variables.VariableNamerDefault;
import com.decompiler.bytecode.opcode.DecodedLookupSwitch;
import com.decompiler.bytecode.opcode.DecodedTableSwitch;
import com.decompiler.bytecode.opcode.JVMInstr;
import com.decompiler.bytecode.opcode.OperationFactoryMultiANewArray;
...
import com.decompiler.util.*;
import com.decompiler.util.bytestream.BaseByteData;
import com.decompiler.util.collections.*;
import com.decompiler.util.functors.BinaryPredicate;
...
import com.decompiler.util.graph.GraphVisitorFIFO;
import com.decompiler.util.lambda.LambdaUtils;
...
public class Op02WithProcessedDataAndRefs implements Dumpable, Graph<Op02WithProcessedDataAndRefs> {
    ...
    private void removeTarget(Op02WithProcessedDataAndRefs node) {
        if (!targets.remove(node)) {
            throw new ConfusedDecompilerException("Invalid target, tried to remove " + node + "\nfrom " + this + "\nbut was not a target.");
        }
    }
    ...
    public void replaceTarget(Op02WithProcessedDataAndRefs oldTarget, Op02WithProcessedDataAndRefs newTarget) {
        ...
        if (index == -1) throw new ConfusedDecompilerException("Invalid target");
    ...
    private void populateStackInfo(StackSim stackSim, Method method, Set<DecompilerComment> comments, LinkedList<Pair<StackSim, Op02WithProcessedDataAndRefs>> next) {
        ...
        if (stackDepthBeforeExecution != -1) {
            ...
            if (stackSim.getDepth() != stackDepthBeforeExecution) {
                throw new ConfusedDecompilerException("Invalid stack depths @ " + this + " : trying to set " + stackSim.getDepth() + " previously set to " + stackDepthBeforeExecution);
            }
            ...
            if (alsoConsumed.size() != stackConsumed.size()) {
                throw new ConfusedDecompilerException("Unexpected stack sizes on merge");
            }
    ...
    private ExceptionGroup getSingleExceptionGroup() {
        if (exceptionGroups.size() != 1) {
            throw new ConfusedDecompilerException("Only expecting statement to be tagged with 1 exceptionGroup");
        }
...
//                    return new Nop();
            ...
            default:
                throw new ConfusedDecompilerException("Not implemented - conversion to statement from " + instr);
        }
    }

    public Pair<Integer, Integer> getIincInfo() {
        if (instr != JVMInstr.IINC) throw new ConfusedDecompilerException("Should be IINC");
        int variableIndex = getInstrArgU1(0);
        int incrAmount = getInstrArgByte(1);
    ...
    private Expression getLiteralConstantPoolEntry(Method m, ConstantPoolEntry cpe, DecompilerComments comments) {
        ...
        throw new ConfusedDecompilerException("Constant pool entry is neither literal, dynamic literal, method handle or method type.");
    }
    ...
    private Expression getDynamicLiteral(Method method, ConstantPoolEntryDynamicInfo cpe, DecompilerComments comments) {
        ...
        if (!(s instanceof AssignmentSimple)) {
            throw new ConfusedDecompilerException("Expected a result from a dynamic literal");
        }
...
//            throw new ConfusedCFRException("Can't compare these exception groups.");
    ...
    private static Op02WithProcessedDataAndRefs adjustOrdering(
        ...
        if (insertionPos == 0) { // start
            ...
            /* Anything which was */
            throw new ConfusedDecompilerException("EEk.");
        }
    ...
    private static int getLastIndex(Map<Integer, Integer> lutByOffset, int op2count, long codeLength, int offset) {
        ...
        if (iinclusiveLastIndex == null) {
            if (offset == codeLength) {
                ...
                throw new ConfusedDecompilerException("Last index of " + offset + " is not a valid entry into the code block");
            }
        }
            ...
            for (ExceptionGroup.Entry exceptionEntry : rawes) {
                ...
                if (handlerIndex <= originalIndex) {
                    if (!options.getOption(OptionsImpl.LENIENT)) {
                        throw new ConfusedDecompilerException("Back jump on a try block " + exceptionEntry);
                    }
                }
...
//            }
                ...
                if (tryTargetSources.isEmpty()) {
                    ...
                    for (Op02WithProcessedDataAndRefs source : tryTargetSources) {
                        if (source.getInstr() == JVMInstr.FAKE_CATCH) {
                            ...
                            if (!options.getOption(OptionsImpl.LENIENT)) {
                                throw new ConfusedDecompilerException("non catch before exception catch block");
                            }
                        }
                    }
                    if (preCatchOp == null) {
    ...
    private static void tieUpRelations(List<Op02WithProcessedDataAndRefs> out, List<Op02WithProcessedDataAndRefs> in, Map<Op02WithProcessedDataAndRefs, Op02WithProcessedDataAndRefs> map) {
        ...
        for (Op02WithProcessedDataAndRefs i : in) {
            ...
            if (mapped == null) {
                throw new ConfusedDecompilerException("Missing node tying up JSR block");
            }
            out.add(mapped);
        }
    }
