...
import com.decompiler.bytecode.analysis.opgraph.op3rewriters.CompareByIndex;
import com.decompiler.bytecode.analysis.opgraph.op3rewriters.TypeFilter;
...
import com.decompiler.bytecode.analysis.parse.statement.SwitchStatement;
...
import com.decompiler.bytecode.analysis.parse.utils.LValueAssignmentAndAliasCondenser;
...
import com.decompiler.bytecode.analysis.parse.utils.LValueUsageCollectorSimpleRW;
import com.decompiler.bytecode.analysis.parse.utils.SSAIdent;
...
import com.decompiler.util.collections.UniqueSeenQueue;
...
public class Op03SimpleStatement implements MutableGraph<Op03SimpleStatement>, Dumpable, StatementContainer<Statement>, IndexedStatement {
    ...
    public void addSource(Op03SimpleStatement source) {
        if (source == null) throw new ConfusedDecompilerException("Null source being added.");
        sources.add(source);
    }
    ...
    public Statement getTargetStatement(int idx) {
        if (targets.size() <= idx) {
            throw new ConfusedDecompilerException("Trying to get invalid target " + idx);
        }
        Op03SimpleStatement target = targets.get(idx);
        Statement statement = target.getStatement();
        if (statement == null) throw new ConfusedDecompilerException("Invalid target statement");
        return statement;
    }
    ...
    public void nopOut() {
        ...
        if (this.targets.size() != 1) {
            throw new ConfusedDecompilerException("Trying to nopOut a node with multiple targets");
        }
        this.containedStatement = new Nop();
        containedStatement.setContainer(this);
    ...
    public void splice(Op03SimpleStatement newSource) {
        if (newSource.targets.size() != 1) {
            throw new ConfusedDecompilerException("Can't splice (bad targets)");
        }
        if (sources.size() != 1) {
            throw new ConfusedDecompilerException("Can't splice (bad sources)");
        }
        if (targets.size() != 1) {
            throw new ConfusedDecompilerException("Can't splice (bad new target)");
        }
    ...
    public void replaceTarget(Op03SimpleStatement oldTarget, Op03SimpleStatement newTarget) {
        ...
        if (index == -1) {
            throw new ConfusedDecompilerException("Invalid target");
        }
    ...
    public void removeSource(Op03SimpleStatement oldSource) {
        if (!sources.remove(oldSource)) {
            throw new ConfusedDecompilerException("Invalid source, tried to remove " + oldSource + "\nfrom " + this + "\nbut was not a source.");
        }
    }

    public void removeTarget(Op03SimpleStatement oldTarget) {
        if (containedStatement instanceof GotoStatement) {
            throw new ConfusedDecompilerException("Removing goto target");
        }
        if (!targets.remove(oldTarget)) {
            throw new ConfusedDecompilerException("Invalid target, tried to remove " + oldTarget + "\nfrom " + this + "\nbut was not a target.");
        }
    }
    ...
    public void markBlockStatement(BlockIdentifier blockIdentifier, Op03SimpleStatement lastInBlock, Op03SimpleStatement blockEnd, List<Op03SimpleStatement> statements) {
        if (thisComparisonBlock != null) {
            throw new ConfusedDecompilerException("Statement marked as the start of multiple blocks");
        }
        ...
        switch (blockIdentifier.getBlockType()) {
            ...
            case SIMPLE_IF_TAKEN:
                throw new ConfusedDecompilerException("Shouldn't be marking the comparison of an IF");
            default:
                throw new ConfusedDecompilerException("Don't know how to start a block like this");
        }
    }
    ...
    public void markFirstStatementInBlock(BlockIdentifier blockIdentifier) {
        if (this.firstStatementInThisBlock != null && this.firstStatementInThisBlock != blockIdentifier
            ...
            throw new ConfusedDecompilerException("Statement already marked as first in another block");
        }
