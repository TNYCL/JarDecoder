...
import com.decompiler.bytecode.analysis.opgraph.op4rewriters.*;
import com.decompiler.bytecode.analysis.opgraph.op4rewriters.checker.Op04Checker;
import com.decompiler.bytecode.analysis.opgraph.op4rewriters.transformers.*;
...
import com.decompiler.bytecode.analysis.parse.rewriters.ConstantFoldingRewriter;
...
import com.decompiler.bytecode.analysis.parse.utils.scope.AbstractLValueScopeDiscoverer;
import com.decompiler.bytecode.analysis.parse.utils.scope.LValueScopeDiscoverImpl;
import com.decompiler.bytecode.analysis.parse.utils.scope.LocalClassScopeDiscoverImpl;
...
import com.decompiler.util.output.LoggerFactory;
...
//    private final int idx = id++;
    ...
    public void replaceTarget(Op04StructuredStatement from, Op04StructuredStatement to) {
        ...
        if (index == -1) {
            throw new ConfusedDecompilerException("Invalid target.  Trying to replace " + from + " -> " + to);
        }
    ...
    public void removeLastContinue(BlockIdentifier block) {
        if (structuredStatement instanceof Block) {
            ...
            throw new ConfusedDecompilerException("Trying to remove last continue, but statement isn't block");
        }
    }
    ...
    public void removeLastGoto() {
        if (structuredStatement instanceof Block) {
            ...
            throw new ConfusedDecompilerException("Trying to remove last goto, but statement isn't a block!");
        }
    }
...
//            logger.warning("From " + wasIn + " to " + nowIn + " = " + nowCopy);
            throw new ConfusedDecompilerException("Started " + nowCopy.size() + " blocks at once");
        }
    ...
    private static void processEndingBlocks(
        ...
        while (!endOfTheseBlocks.isEmpty()) {
            if (mutableProcessingBlockState.currentBlockIdentifier == null) {
                throw new ConfusedDecompilerException("Trying to end block, but not in any!");
            }
            // Leaving a block, but
            if (!endOfTheseBlocks.remove(mutableProcessingBlockState.currentBlockIdentifier)) {
                throw new ConfusedDecompilerException("Tried to end blocks " + endOfTheseBlocks + ", but top level block is " + mutableProcessingBlockState.currentBlockIdentifier);
            }
